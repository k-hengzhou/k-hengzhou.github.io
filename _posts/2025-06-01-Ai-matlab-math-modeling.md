...
categories: ai
tags: [AI, 编程助手]
description: "面向程序员与学生的 AI 编程助手选型指南：深入对比 GitHub Copilot、Cursor、Cline 三款主流工具的功能、安装配置、收费方案与 Act/Plan 交互模式，附 MATLAB 绘图实战案例与使用建议，帮你挑到最适合自己的 AI 编程搭档"
...
DeepSeek 使用按量计费模式，价格非常便宜。费用根据实际消耗的 token 数量计算，一个 token 可以是单词、数字或标点符号。

<img src="/images/post/deepseek.png" width="100%" alt="vscode7"  style="center"/>

具体来说：DeepSeek 目前提供两个主要模型，按每 1M tokens 计费：

| 模型                  | 输入（缓存命中） | 输入（缓存未命中） | 输出 |
| --------------------- | :--------------: | :----------------: | :--: |
| **deepseek-v4-flash** |      ¥0.02       |        ¥1.0        | ¥2.0 |
| **deepseek-v4-pro**   |      ¥0.026      |        ¥3.1        | ¥6.3 |

> 注：旧模型名称 `deepseek-chat` 和 `deepseek-reasoner` 将于 2026/07/24 后废弃，分别对应 `deepseek-v4-flash` 的非思考模式和思考模式。

**计费说明**：

1. **按量计费**：费用 = token 数量 × 单价，直接从账户余额扣除
2. **缓存优惠**：命中缓存时输入价格大幅降低（约 50 倍优惠）
3. **超大上下文**：支持 1M 上下文长度，最大输出 384K tokens
4. **并发限制**：Flash 模型 2500 并发，Pro 模型 500 并发
...
以一个典型的 Cline 编程任务为例，假设输入 10K tokens、输出 5K tokens（使用 Flash 模型）：

- 输入费用：10K × ¥1.0/1M = ¥0.01
- 输出费用：5K × ¥2.0/1M = ¥0.01
- **总计约 ¥0.02（不到 2 分钱）**

相比旧版积分制，新版按量计费更加透明灵活，对于日常使用来说成本极低。
